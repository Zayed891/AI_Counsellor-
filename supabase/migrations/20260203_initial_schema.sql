...
begin
    ...
    if not exists (select 1 from information_schema.columns where table_name = 'profiles' and column_name = 'previous_degree') then
        alter table public.profiles add column previous_degree text;
    end if;
